...
import { addNewPet } from "../../lib/api/admin";

const NewPetForm = () => {
  const [name, setName] = useState("");
  const [type, setType] = useState("Dog");
  const [breed, setBreed] = useState("");
  const [color, setColor] = useState("undef");
  ...
  const selectFile = (event) => {
    if (event.target.files.length > 1) {
      setError({ other: "Cannot add more than one file" });
    } else {
  ...
  const handleSubmit = (e) => {
    ...
    addNewPet(bodyFormData).then((response) => {
      if (!response) {
